...
#   2. Changelog current     - All commits must be documented in CHANGELOG.md
#                              (commits that only touch CHANGELOG.md are excluded)
#   3. On main branch        - Warning if not on main (can override)
#
# Changelog validation:
#   - Extracts 7-char commit SHAs from CHANGELOG.md commit links
#   - Finds commits since the latest documented SHA
#   - Fails if any commits (except CHANGELOG-only) are undocumented
#   - On failure, directs user to run: release-engineer:update-changelog
#
# Usage:
...
if [ ${#UNDOCUMENTED[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Please update ${SOURCE_PREFIX}/CHANGELOG.md with these changes before publishing."
    echo ""
