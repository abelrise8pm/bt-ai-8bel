...
└── plugins-dev/            # Where forked skills will live
```
...
```
.claude/plugins-dev/local-skills/
├── .claude-plugin/
│   └── plugin.json
└── skills/
    └── {skill-name}-local/
...
```
.claude/plugins-dev/{existing-plugin}/skills/{skill-name}-local/
```
...
1. **Create plugin structure** (if new plugin):
   ...
   Create `.claude/plugins-dev/local-skills/.claude-plugin/plugin.json`:
   ```json
   {
     "name": "local-skills",
     "version": "0.1.0-dev",
...
4. **Add to local marketplace** (if new plugin):
   ...
   ```json
   {
     "plugins": [
       {
         "name": "local-skills",
         "source": "./plugins-dev/local-skills",
         "description": "Local skill development and customization"
       }
     ]
   }
...
**Location:** `.claude/plugins-dev/local-skills/skills/{skill-name}-local/SKILL.md`
...
4. Edit `.claude/plugins-dev/local-skills/skills/{skill-name}-local/SKILL.md` to customize
