---
name: claude-whisperer-beta-contribute-skill
...
- By path: "contribute the skill in .claude/skills/local-skills/skills/assess-story-local"
...
1. Look in `.claude/skills/` for the skill
...
```
.claude/skills/{original-plugin}/skills/{skill-name}/SKILL.md
```
...
**Target location:** `.claude/skills/{plugin}/skills/{skill-name}/SKILL.md`
...
3. **Create/update the skill file:**
   ...
   \`\`\`
   .claude/skills/{plugin}/skills/{skill-name}/SKILL.md
   \`\`\`

4. **Commit and push:**
   \`\`\`bash
   git add .claude/skills/{plugin}/skills/{skill-name}
